...
$("#calculateButton").on("click", function() {
    ...
    var wpm = Math.floor((totalTypedCharacters / fiveCharacters) / minutes);
    var accuracy = accuracyCheck(typedText, targetText);
    ...
    for (i = 0; i < targetText.length; i++) {
        if (typedText[i] === targetText[i]) {
           highlightedText += '<span class="match">' + targetText[i] + '</span>';
            ...
            highlightedText += '<span class="dont-match">' + targetText[i] + '</span>';
        }
    }
...
function intervalTimer() {
    ...
    var remainedSeconds = seconds % 60;

    if (seconds >= 60) {
        $("#timer").text("Your elapsed time is " + minutes + " minutes and " + remainedSeconds + " seconds");
    } else {
        $("#timer").text("Your elapsed time is " + seconds + " seconds");
    }
}
...
function resetAll() {
    ...
    startTime = null;
    $("#timer").text("Your elapsed time is 0 seconds");
